...
w2b <- c("#FFFFFF", "#0000FF")   #white to blue
w2borig <- c("#FFFFFF", "#BDC9E1", "#74A9CF", "#2B8CBE","#045A8D")   #white to blue
b2w <- c("#045A8D", "#2B8CBE", "#74A9CF", "#BDC9E1", "#F1EEF6")   #blue to white
 ...
 layout = c(1, nyr%%7),
   ...
   scales = list(
     x = list(
               ...
               labels = c("Jan", "Feb", "Mrz", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez"),
   ...
   xlab="" ,
   ylab="",
   colorkey= list(col = calendar.pal(ncolors), width = 0.6, height = 0.5, labels=list(seq(0, ncolors), cex = 1.25)),
   subscripts=TRUE
    ) )
...
setwd("~/zeugs/codeforMS/fahrrad/traffic-dynamics-shiny/shinyapp")
...
bikes <- dbGetQuery(conn = con, 
                  ...
                  FROM bikes WHERE location == 'Neutor' AND count != ''")
...
bikes_weather <-
	...
	group_by(date) %>%
	summarise(rainhours = as.integer(sum(weather == "Gewitter" |
									weather == "Regen" |
									weather == "Schnee" |
									weather == "Schneeregen")))
...
calendarHeat(bikes_weather$date, 
						 bikes_weather$rainhours, 
						 color = "w2b", 
						 ncolors = 25,
						 title = "Anzahl Regenstunden zwischen 0 und 24 Uhr in Münster")
